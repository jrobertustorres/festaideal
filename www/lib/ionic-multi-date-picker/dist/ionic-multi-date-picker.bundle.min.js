...
(function(module) {
    ...
    module.run(['$templateCache', function($templateCache) {
        $templateCache.put('ionic-multi-date-picker-popup.html',
            ...
            '<div class="title ng-hide" ng-show="header.length > 0">' +
            '<h3 ng-class=headerClass ng-bind=header></h3></div><ng-include src="\'template-body.html\'"></ng-include>' +
            '<div class="btns-right-popup ng-hide" ng-show=!btnsIsNative><button class=button ng-repeat="btn in btns" ng-click=btn.onTap() ng-disabled="btn.sType == \'ok\' && errors.len > 0" ng-class=btn.type>{{ btn.text }}</button></div><div class=clear></div></div>');
    }]);
})();
...
(function() {
    ...
    function IonicMultiDatePicker($ionicGesture, $ionicModal, $ionicPopup, $timeout, IonicMultiDatePickerService, $scope) {
        return {
            restrict: 'AE',
            replace: false,
            ...
            link: function(scope, element, attrs) {
                ...
                function btnOk() {
                    // scope.inputObj.callback(scope.openModalAgenda);
